...
# Schedule an async coroutine on a separate thread (for web backend compatibility)
def _schedule(coro):
...
class SwipeVerseApp:
    ...
    async def navigate_to(self, screen_name: str, **kwargs: Any) -> None:
        ...
        if screen_name == "title":
            ...
            self.current_screen = TitleScreen(
                on_start_game=lambda: _schedule(self.navigate_to("game")),
                on_load_config=self._handle_load_config,
                on_settings=lambda: _schedule(self.navigate_to("settings")),
                backstory=backstory,
            )
            ...
            if self.game_state and self.game_logic:
                self.current_screen = GameScreen(
                    ...
                    on_new_game=lambda: _schedule(self.new_game()),
                    on_main_menu=lambda: _schedule(self.navigate_to("title")),
                )
        elif screen_name == "settings":
            self.current_screen = SettingsScreen(
                on_save=self._handle_save_settings,
                on_cancel=lambda: _schedule(self.navigate_to("title")),
        ...
        # Attach page to screen
        try:
            self.current_screen.page = self.page
        except Exception:
            pass
        # Render the new screen by appending the screen instance
        self.page.controls.clear()
        self.page.controls.append(self.current_screen)
        self.page.update()

        # Call did_mount for components that need initialization
        if hasattr(self.current_screen, "did_mount"):
            import inspect
    ...
    def _handle_save_settings(self, settings: Dict[str, Any]) -> None:
        ...
        if game_changed or not self.game_state:
            ...
            _schedule(self._load_and_switch_theme(new_config_path, player_name, difficulty))
        else:
    ...
    def build(self) -> ft.Container:
        ...
        # Initial navigation to title screen
        # Use page.run_async for scheduling coroutine execution
        self.page.run_async(self.navigate_to("title"))

        # Container that fills the page
        return ft.Container(expand=True, content=self.current_screen)
    # Provide Flet Control interface for page.add by implementing add commands
    def _build_add_commands(self, indent: int = 0, index=None, added_controls=None):
        # Delegate add commands to the placeholder container generated by build()
        container = self.build()
        # Delegate the actual command construction
        return container._build_add_commands(indent, index=index, added_controls=added_controls)

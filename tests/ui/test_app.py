...
def mock_flet(mocker):
    ...
    mocker.patch("swipe_verse.ui.app.ft", mock_module)
    mocker.patch("swipe_verse.ui.title_screen.ft", mock_module)
    mocker.patch("swipe_verse.ui.game_screen.ft", mock_module)
    mocker.patch("swipe_verse.ui.settings_screen.ft", mock_module)
...
def mock_services(mocker):
    ...
    mocker.patch("swipe_verse.ui.app.ConfigLoader", return_value=mock_config_loader)

    mock_asset_manager = mocker.MagicMock()
    mocker.patch("swipe_verse.ui.app.AssetManager", return_value=mock_asset_manager)

    mock_image_processor = mocker.MagicMock()
    mocker.patch("swipe_verse.ui.app.ImageProcessor", return_value=mock_image_processor)
...
def mock_screens(mocker):
    ...
    mocker.patch.dict(
        "sys.modules",
        {
            "swipe_verse.ui.title_screen": mocker.MagicMock(),
            "swipe_verse.ui.game_screen": mocker.MagicMock(),
            "swipe_verse.ui.settings_screen": mocker.MagicMock(),
        },
    )
    ...
    title_screen_module = sys.modules["swipe_verse.ui.title_screen"]
    title_screen_module.TitleScreen = mocker.MagicMock(return_value=mock_title_screen)

    # Set up GameScreen mock
    game_screen_module = sys.modules["swipe_verse.ui.game_screen"]
    game_screen_module.GameScreen = mocker.MagicMock(return_value=mock_game_screen)

    # Set up SettingsScreen mock
    settings_screen_module = sys.modules["swipe_verse.ui.settings_screen"]
    settings_screen_module.SettingsScreen = mocker.MagicMock(
        return_value=mock_settings_screen
    )
...
def mock_game_state(mocker):
    ...
    mock_state_class = mocker.patch("swipe_verse.ui.app.GameState")
    mock_state_class.new_game.return_value = mock_state

    mock_logic = mocker.MagicMock()
    mock_logic_class = mocker.patch("swipe_verse.ui.app.GameLogic")
...
def app(mock_flet, mock_services, mock_game_state, mock_page, mocker):
    ...
    mocker.patch("swipe_verse.ui.app.Path", autospec=True)
...
async def test_load_config_success(app, mock_services, mock_game_state, mocker):
    ...
    mocker.patch(
        "swipe_verse.ui.app.GameState.new_game", return_value=mock_game_state["state"]
    )

    # Set up GameLogic mock
    mocker.patch("swipe_verse.ui.app.GameLogic", return_value=mock_game_state["logic"])
...
@pytest.mark.asyncio
async def test_navigate_to_game_with_state(
    app, mock_page, mock_screens, mock_game_state, mocker
):
...
async def test_new_game(app, mock_game_state, mocker):
    ...
    mock_game_state["state_class"].new_game.assert_called_with(
        app.game_logic.config, player_name="Test Player", difficulty="hard"
    )

    # Verify new game logic was created
    mock_game_state["logic_class"].assert_called_with(
        app.game_state, app.game_logic.config
    )
